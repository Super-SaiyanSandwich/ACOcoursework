...
class Weight:
    def __init__(self, value, totalBins, evapRate):
        ...
        for i in range(totalBins):
            x = binH.Bin(i,evapRate,totalBins)
            self.bins.append(x)
    ...
    def chooseRandBin(self, path, bins):
        return self.bins[path].chooseRandBin(bins, self.value)
    ...
    def chooseInitialBin(self):
        return self.bins[0].chooseRandBin([0]*len(self.bins),self.value)
